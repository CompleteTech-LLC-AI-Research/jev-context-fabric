...
from jev_context import paging  # noqa: E402
...
class StageTests(BusHomeCase):
    """This package's own stage entry point."""

    def test_request_schema_is_enforced(self):
        import io
        for bad in ({"schema": "wrong", "op": "transform", "messages": []},
                    {"schema": bus.STAGE_SCHEMA, "op": "delete-everything", "messages": []},
                    {"schema": bus.STAGE_SCHEMA, "op": "transform", "messages": "nope"}):
            with self.assertRaises(bus.BusError):
                bus.read_stage_request(io.BytesIO(json.dumps(bad).encode()))

    def test_host_maps_onto_this_packages_harness_string(self):
        self.assertEqual(paging.harness_for_host("opencode", "v2"), "opencode-v2")
        self.assertEqual(paging.harness_for_host("opencode", ""), "opencode-v1")
        self.assertEqual(paging.harness_for_host("pi"), "pi")
        self.assertEqual(paging.harness_for_host("hermes"), "hermes")
        self.assertEqual(paging.harness_for_host("something-else"), "generic")

    def test_bus_only_hosts_claim_support_only_when_registered(self):
        # This package installs no native transform for pi/hermes; it must not claim it can
        # prune there merely because it knows the name.
        self.assertFalse(paging.can_mutate("pi"))
        self.assertFalse(paging.can_mutate("hermes"))
        bus.register(PACKAGE, [self.own_stage()], {})
        self.assertTrue(paging.can_mutate("pi"))
        self.assertTrue(paging.can_mutate("hermes"))

    def test_hosts_with_an_own_adapter_need_no_bus(self):
        for harness in ("generic", "opencode-v1", "opencode-v2"):
            self.assertTrue(paging.can_mutate(harness))

    def test_hosts_with_no_transform_api_are_never_mutable(self):
        for harness in ("codex", "claude-code", "openclaw", "gemini", "cursor", "copilot"):
            self.assertFalse(paging.can_mutate(harness))
...
class ContractDriftTests(unittest.TestCase):
    ...
    def files(self):
        ...
        return {"bus.py": root / "src" / "jev_context" / "bus.py",
